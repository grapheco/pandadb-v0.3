...
import cn.pandadb.kernel.{DistributedPDBMetaData}
 ...
 * @Modified By:
 */
class DistributedSingleNodeFileImporter(file: File, importCmd: ImportCmd, globalArgs: DistributedGlobalArgs) extends SingleFileImporter {
    ...
    headLine.zipWithIndex.map(item => {
      if(item._2 == idIndex || item._2 == labelIndex){
        if(item._1.split(":")(0).length == 0) {
          ...
          val propId = DistributedPDBMetaData.getPropId(pair(0))
          val propType = "string"
  ...
  private def _wrapNode(lineArr: Array[String]): (Long, Array[Int], Map[Int, Any]) = {
    ...
    val labelIds: Array[Int] = labels.map(label => DistributedPDBMetaData.getLabelId(label))
