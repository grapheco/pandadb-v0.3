...
/**
  ...
  * @Modified By:
  */
class DistributedSingleRelationFileImporter(file: File, importCmd: ImportCmd, globalArgs: DistributedGlobalArgs) extends SingleFileImporter {
  ...
  private def _wrapEdge(lineArr: Array[String]): StoredRelationWithProperty = {
    ...
    val edgeType: Int = DistributedPDBMetaData.getTypeId(lineArr(labelIndex))
