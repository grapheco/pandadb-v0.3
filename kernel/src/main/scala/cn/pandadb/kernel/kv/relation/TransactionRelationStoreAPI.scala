...
class TransactionRelationStoreAPI(relationDB: TransactionDB,
  ...
  override def getRelationIdsByRelationType(relationTypeId: Int): Iterator[Long] = relationLabelStore.getRelations(relationTypeId)
  ...
  override def allRelations(withProperty: Boolean): Iterator[StoredRelation] = {
