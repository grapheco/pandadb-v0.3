...
import org.grapheco.pandadb.kernel.distribute.meta.{NameMapping, PropertyNameStore}
import org.grapheco.pandadb.kernel.distribute.node.{DistributedNodeStoreSPI, NodeStoreAPI}
...
class NodeStoreAPITest {
  ...
  def init(): Unit = {
    ...
    api = new NodeStoreAPI(db, new PropertyNameStore(db, new UDPClientManager(udpClient)))
